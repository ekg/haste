...
using haste::v0::lstm_silu::ForwardPass;
using haste::v0::lstm_silu::BackwardPass;

using torch::Tensor;

// LSTM + SiLU Selectivity Gate Forward
//
...
//   y = h_lstm * gate
//
// Returns: {h (hidden states), c (cell states), y (gated outputs), cache, gate_pre}
std::vector<Tensor> lstm_silu_forward(
    bool training,
    Tensor x,          // [T, N, C] input
    Tensor h0,         // [N, H] initial hidden state
    Tensor c0,         // [N, H] initial cell state
    Tensor W,          // [C, H*4] LSTM input weights
    Tensor R,          // [H, H*4] LSTM recurrent weights
    Tensor b,          // [H*4] LSTM bias
    ...
    Tensor c = torch::empty({ time_steps + 1, batch_size, hidden_size }, options);
    ...
    Tensor tmp_gate = torch::empty({ batch_size, hidden_size }, options);

    h[0] = h0;
    c[0] = c0;

    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
        x.scalar_type(), "lstm_silu_forward", ([&] {
    ...
    return { h, c, y, cache, gate_pre };
}

// LSTM + SiLU Selectivity Gate Backward
std::vector<Tensor> lstm_silu_backward(
    Tensor x_t,        // [C, T, N] transposed input
    Tensor W_t,        // [H*4, C] transposed LSTM input weights
    Tensor R_t,        // [H*4, H] transposed LSTM recurrent weights
    Tensor b,          // [H*4] LSTM bias
    ...
    Tensor c,          // [T+1, N, H] cell states from forward
    Tensor y,          // [T, N, H] gated outputs from forward
    Tensor cache,      // [T, N, H*4] LSTM activations from forward
    ...
    // Temporaries
    Tensor dp = torch::empty({ time_steps, batch_size, hidden_size * 4 }, options);
    Tensor dh_lstm = torch::empty({ time_steps, batch_size, hidden_size }, options);
    Tensor tmp_dgate = torch::empty({ time_steps, batch_size, hidden_size }, options);

    // h_out is h[1:], which represents the LSTM output at each timestep
    Tensor h_out = h.slice(0, 1, time_steps + 1);

    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
        x_t.scalar_type(), "lstm_silu_backward", ([&] {
        ...
        backward.Run(
            ...
            ptr<scalar_t>(dc),
            ptr<scalar_t>(dp),
            ptr<scalar_t>(dh_lstm),
            ptr<scalar_t>(tmp_dgate));
    }));

    return { dx, dh, dc, dW, dR, db, dWg_x, dWg_h, dbg };
}

}  // anonymous namespace

void lstm_silu_init(py::module& m) {
    m.def("lstm_silu_forward", &lstm_silu_forward, "LSTM+SiLU forward",
          py::call_guard<py::gil_scoped_release>());
    m.def("lstm_silu_backward", &lstm_silu_backward, "LSTM+SiLU backward",
          py::call_guard<py::gil_scoped_release>());
}
